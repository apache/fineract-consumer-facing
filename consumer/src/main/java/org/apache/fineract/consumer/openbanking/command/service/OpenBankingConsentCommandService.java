...
public interface OpenBankingConsentCommandService {

    OpenBankingConsentCommandData create(Jwt jwt, CreateOpenBankingConsentCommand command);

    void authorise(UUID consentId, UUID userId);

    void reject(UUID consentId, UUID userId);

    OpenBankingConsentCommandData revoke(Jwt jwt, UUID consentId);
}
