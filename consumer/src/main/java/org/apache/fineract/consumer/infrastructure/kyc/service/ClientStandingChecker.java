...
public class ClientStandingChecker {

    private static final long ACTIVE_STATUS_ID = 300L;

    private final ClientApi clientApi;

    public boolean isActive(Long fineractClientId) {
        try {
            GetClientsClientIdResponse client = clientApi.retrieveOneClient(fineractClientId, null);
            GetClientsClientIdStatus status = client == null ? null : client.getStatus();
            return status != null && status.getId() != null && status.getId() == ACTIVE_STATUS_ID;
