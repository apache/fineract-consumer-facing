...
public class OpenBankingUserConsentCommandController {

    private final OpenBankingConsentCommandService openBankingConsentCommandService;

    @Operation(operationId = "revokeConsent")
    @PostMapping("/{consentId}/revoke")
    public OpenBankingConsentCommandData revokeConsent(@AuthenticationPrincipal Jwt jwt,
            @PathVariable UUID consentId) {
        return openBankingConsentCommandService.revoke(jwt, consentId);
    }
}
