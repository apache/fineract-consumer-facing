...
@Component({
  ...
  template: `
    <app-page-header [title]="'loans.detail.title' | translate">
      @if (repayable()) {
        <ion-button [routerLink]="['/loans', loanId, 'repay']">
          <ion-icon slot="start" name="cash" aria-hidden="true" />
          {{ 'loans.detail.repayCta' | translate }}
        </ion-button>
      }
    </app-page-header>
    ...
    @if (store.selected(); as loan) {
      <ion-card>
        ...
        <ion-card-content>
          ...
          <p>
            {{ 'loans.detail.annualInterestRateLabel' | translate }} {{ loan.annualInterestRate }}%
          </p>
          <p>
            {{ 'loans.detail.nextDueLabel' | translate }}
            {{ loan.nextDueAmount | currency: loan.currency }}
        ...
        <div class="table-scroll">
          <table cdk-table [dataSource]="store.guarantors()">
            ...
            <ng-container cdkColumnDef="guarantorType">
              ...
              <td cdk-cell *cdkCellDef="let row">{{ row.guarantorType | translate }}</td>
            </ng-container>
            <ng-container cdkColumnDef="relationship">
              <th cdk-header-cell *cdkHeaderCellDef>
                {{ 'common.table.relationship' | translate }}
              </th>
            ...
            <tr class="empty-row" *cdkNoDataRow>
              <td [attr.colspan]="guarantorColumns.length">
                {{ 'loans.detail.noGuarantors' | translate }}
              </td>
            </tr>
          </table>
        </div>
      ...
      <ion-card-content>
        <form class="filter" [formGroup]="filterForm" (ngSubmit)="applyFilter()">
          ...
          <div class="field">
            ...
            <ion-datetime-button datetime="loansFromDate">
          ...
          <div class="field">
            ...
            <ion-datetime-button datetime="loansToDate">
              <span slot="date-target">
        ...
        <div class="table-scroll">
          <table cdk-table [dataSource]="store.transactions()">
            ...
            <ng-container cdkColumnDef="outstandingLoanBalance">
              ...
              <td cdk-cell *cdkCellDef="let row" class="num">
                {{ row.outstandingLoanBalance | currency: row.currency }}
              </td>
            </ng-container>
...
export class LoansDetailComponent {
  ...
  protected readonly repayable = computed(() => this.store.selected()?.active === true);
  ...
  constructor() {
    ...
    this.store.loadTransactions(this.loanId, { page: 0, size: 20 });
  }
